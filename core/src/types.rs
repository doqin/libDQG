...
impl KeyCode {
    ...
    /// Parses a key name matching this enum's own variant identifiers (the same strings
    /// its derived `Debug` output prints), which happen to follow the W3C UI Events
    /// `KeyboardEvent.code` convention (e.g. "KeyW", "ArrowUp", "Space", "ShiftLeft") —
    /// used by `core::scripting` so a `.rhai` script can reference a key by name.
